  ...
  function(
    ...
    function addMember(membership) {
      return GroupMemberService.addMember($scope.groupId, membership).catch(function(error) {
        ...
        $alert.error(
          'Cannot add member with id: ' + membership.memberId + '. Server error: "' + error.error + '".',
          $rootScope
        );
      });
    }
    ...
    function checkGoupExists(groupId) {
      return GroupService.findById(groupId).catch(function(error) {
        ...
        members.map(function(memberId) {
          ...
          if ($scope.membershipType === 'user') {
            ...
            if ($scope.groupId === memberId) {
              hasSubmissionErrors = true;
              $alert.error(
                'Adding a group with id `' + memberId + '` as a sub-group of itself is not allowed.',
                $rootScope
              );
              return $q.resolve();
            }
            ...
            return checkGoupExists(memberId)
              .then(function() {
                return addMember({
                  memberId: memberId,
                  membershipType: $scope.membershipType
